...
<div class={`alert max-w-xs shadow-lg transition-all duration-200 ${fail && 'alert-error'} ${success && 'alert-success'} ${fail || success ?  'opacity-100 my-5' : 'opacity-0 -mt-10'}`}>
    <div class="unalert">
      {#if fail}
        <Error></Error>
      {:else if success}
...
<form action="#/checkout" class={`flex flex-col-reverse lg:flex-row md:justify-between bg-white shadow-xl rounded-box p-5 mb-5`}>
    <div class="lg:mr-10">
        ...
        <button type="submit" class={`${!valid && 'btn-disabled'} btn btn-primary mt-5 w-full`}
        on:click={onCheckout}
        >Checkout</button>
    </div>
    <div class="mb-5">
