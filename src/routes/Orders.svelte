...
<div class={`${!loading ? 'hidden' : 'flex'} absolute z-0 w-full h-full items-center justify-center`}>
        ...
        {#each Object.entries(data) as [id, order]}
            <li class="flex flex-col rounded-box shadow mb-5 last:mb-0">
                ...
                <div class="bg-white h-auto p-5 md:p-0 flex flex-col md:flex-row justify-between items-center md:items-start">
                    <img class="md:flex w-40 h-40 md:w-auto rounded-box md:rounded-tl-none" src={Object.values(order.basket)[0].image} alt="">
                    <div class="mt-7 md:ml-5 text-xl font-semibold w-full text-center">
                        {(()=>{
                            let text = '';
                    ...
                    <a class="btn btn-primary md:mr-5 mt-5" href={`#/orders/${id}`}>View Details</a>
                </div>
            </li>
        {/each}
    </ul>
